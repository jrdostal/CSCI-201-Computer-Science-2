...
void Library::loadData() {
    ...
    // parse and create Patron objects
    // load transaction data from file
    std::ifstream transactionFile("transactions.txt");
    if (!transactionFile) throw std::runtime_error("Failed to open transactions file.");
...
void Library::saveData() {
    ...
    for (const auto* t : transactions) {
        transactionFile << t->patronId << "," << t->bookTitle << "," << t->date << endl;
    }
};
