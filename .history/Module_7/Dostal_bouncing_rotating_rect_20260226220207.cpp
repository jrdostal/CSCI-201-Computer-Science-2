...
int main() {
    ...
    while (window.isOpen()) {
        ...
        if (boundingBox.position.x <= 0 || (boundingBox.position.x + (boundingBox.size.x - 1.0f)) >= Window_Width)
        {
            speedX = -speedX;
        }
        if (boundingBox.position.y <= 0 || (boundingBox.position.y + (boundingBox.size.y - 1.0f)) >= Window_Height)
        {
            speedY = -speedY;
        }
