...
int main() {
    ...
    while (window.isOpen()) {
        ...
        if ((boundingBox.position.x - (boundingBox.size.x * 2)) <= 0 || (boundingBox.position.x + (boundingBox.size.x * 2)) >= Window_Width)
        {
            speedX = -speedX;
        }
        ...
        if ((boundingBox.position.y - (boundingBox.size.y * 2)) <= 0 || (boundingBox.position.y + (boundingBox.size.y * 2)) >= Window_Height)
        {
            speedY = -speedY;
        }
