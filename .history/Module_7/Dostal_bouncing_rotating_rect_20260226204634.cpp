...
int main() {
    ...
    rectangle1.setPosition({60.0f, 60.0f});
    ...
    while (window.isOpen()) {
        ...
        if (pos.x - Rectangle_Width / 1.5f <= 0 || pos.x + Rectangle_Width / 1.5f >= Window_Width) {
            // Reverse X direction
            speedX = -speedX;
        }

        //SECTION - Check for collision with the top and bottom boundaries and reverse direction if necessary.
        if (pos.y - Rectangle_Height / 1.5f <= 0 || pos.y + Rectangle_Height / 1.5f >= Window_Height) {
            // Reverse Y direction
            speedY = -speedY;
        }
