...
int main() {
    ...
    rectangle1.setPosition({(Rectangle_Width / 2.0f) + 4.0f, (Rectangle_Height / 2.0f) + 4.0f});
    ...
    while (window.isOpen()) {
        ...
        if (boundingBox.position.x - 2.0f <= 0 || (boundingBox.size.x + boundingBox.position.x + 2.0f) >= Window_Width)
        if ((boundingBox.position.x - 2.0f <= 0 && speedX < 0) || ((boundingBox.size.x + boundingBox.position.x + 2.0f) >= Window_Width && speedX > 0))
        {
            speedX = -speedX;
        }
        ...
        if (boundingBox.position.y - 2.0f <= 0 || (boundingBox.size.y + boundingBox.position.y + 2.0f) >= Window_Height)
        if ((boundingBox.position.y - 2.0f <= 0 && speedY < 0) || ((boundingBox.size.y + boundingBox.position.y + 2.0f) >= Window_Height && speedY > 0))
        {
            speedY = -speedY;
        }
