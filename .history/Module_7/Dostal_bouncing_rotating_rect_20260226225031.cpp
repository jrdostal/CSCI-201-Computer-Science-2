...
int main() {
    ...
    while (window.isOpen()) {
        ...
        if ((boundingBox.size.x - boundingBox.position.x) <= 0 || (boundingBox.size.x - boundingBox.position.x) >= Window_Width)
        {
            speedX = -speedX;
        }
        ...
        if ((boundingBox.size.y - boundingBox.position.y) <= 0 || (boundingBox.size.y - boundingBox.position.y) >= Window_Height)
        {
            speedY = -speedY;
        }
