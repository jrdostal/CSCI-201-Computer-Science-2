...
int main() {
    ...
    // Position the rectangle at the top-left corner
    ...
    while (window.isOpen()) {
        ...
        if (pos.x - Rectangle_Width / 10.0f < 0 || pos.x + Rectangle_Width / 10.0f > Window_Width) {
            // Reverse X direction
            speedX = -speedX;
        }

        //SECTION - Check for collision with the top and bottom boundaries and reverse direction if necessary.
        if (pos.y - Rectangle_Height / 10.0f < 0 || pos.y + Rectangle_Height / 10.0f > Window_Height) {
            // Reverse Y direction
            speedY = -speedY;
        }
