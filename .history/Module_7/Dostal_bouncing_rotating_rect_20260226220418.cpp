...
int main() {
    ...
    while (window.isOpen()) {
        ...
        if (boundingBox.position.x <= 0 || (boundingBox.position.x + (boundingBox.size.x + 3.5f)) >= Window_Width)
        {
            speedX = -speedX;
        }
        if (boundingBox.position.y <= 0 || (boundingBox.position.y + (boundingBox.size.y + 3.5f)) >= Window_Height)
        {
            speedY = -speedY;
        }
