...
int main() {
    ...
    while (window.isOpen()) {
        ...
        sf::Vector2f point = {Window_Width / 2.0f, Window_Height / 2.0f};
        if (boundingBox.contains(point)) {
            // Collision detected with the point
            // You can handle the collision here (e.g., change color, reverse direction, etc.)
            rectangle1.setFillColor(sf::Color::Red); // Change color to red on collision
        }

        
        //SECTION - Check for collision with the left and right boundaries and reverse direction if necessary.
        if (pos.x - Rectangle_Width == point.x || pos.x + Rectangle_Width == point.x) {
            speedX = -speedX;
        }

        //SECTION - Check for collision with the top and bottom boundaries and reverse direction if necessary.
        if (pos.y - Rectangle_Height == point.y || pos.y + Rectangle_Height == point.y) {
