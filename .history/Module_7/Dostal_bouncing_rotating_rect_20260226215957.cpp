...
int main() {
    ...
    while (window.isOpen()) {
        ...
        if (boundingBox.position.x <= 0 || (boundingBox.position.x + boundingBox.size.x) >= Window_Width - 1.0f)
        {
            speedX = -speedX;
        }
        if (boundingBox.position.y <= 0 || (boundingBox.position.y + boundingBox.size.y) >= Window_Height - 1.0f)
        {
            speedY = -speedY;
        }
