...
char Pair<TheType>::CompareWith(Pair<TheType>* otherPair) {
	if (firstVal < otherPair->firstVal) and (secondVal < otherPair->secondVal){
		return '<';
		}
	else if (firstVal == otherPair->firstVal) and (secondVal == otherPair->secondVal){
		return '=';
	}
	else if (firstVal > otherPair->firstVal) and (secondVal > otherPair->secondVal){
		return '>';
