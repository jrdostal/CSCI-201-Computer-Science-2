...
classDiagram
    ...
    class PrintedBook { -int pageCount }
    class EBook { -double fileSizeMB }
    class Patron { -string name, -int id, -vector~Book*~ borrowedBooks }
    class Transaction { -int patronId, -string bookTitle, -string date }
    class DueDate { -int day, -int month, -int year }
    class DueTime { -int hour, -int minute }

    Library "1" *-- "*" Book : manages
    Library "1" *-- "*" Patron : manages
    Library "1" *-- "*" Transaction : logs
    Book <|-- PrintedBook : inherits
    Book <|-- EBook : inherits
    Patron "1" o-- "*" Book : borrows
    Book *-- DueDate : has
    Book *-- DueTime : has
```
