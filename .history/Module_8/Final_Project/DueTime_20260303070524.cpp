//SECTION - Include necessary header files
#include <iostream>
#include <stdexcept>

using namespace std;

//SECTION - Include project-specific headers for classes and functionality
#include "DueTime.h"

//SECTION - Constructor for the DueTime class
DueTime::DueTime(int h, int m) : hour(h), minute(m) {
    // Validate the time to ensure it is a valid calendar time. If the time is invalid, throw an exception.
    if (h < 0 || h > 23 || m < 0 || m > 59) {
        throw invalid_argument("Invalid time.");
    }
};

//SECTION - Display Time Function
//Display the Time in Hour:Minute format.
void DueTime::displayTime() const {
    cout << hour << ":" << minute;
};

//SECTION - Operator Overloading
//Overload the output stream operator to display the time in a readable format when using cout.
ostream& operator<<(ostream& os, const DueTime& t) {
    os << t.hour << ":" << t.minute;
    return os;
};

//SECTION - Destructor for the DueTime class
// The destructor for the DueTime class. Since there are no dynamically allocated resources, it can be left empty.
DueTime::~DueTime() {};