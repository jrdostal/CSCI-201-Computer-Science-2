...
class Library {
    ...
    public:
        ...
        void saveToFile(const string& filename, const Container& items, Func formatter) {
            ...
            // Validates if the data file opens successfully. If the file doesn't open, throw an exception letting the user know the file is not opened for writing.
            if (!file) throw runtime_error("Failed to open " + filename + " for writing.");

            // Loop to write arguments of function to file
