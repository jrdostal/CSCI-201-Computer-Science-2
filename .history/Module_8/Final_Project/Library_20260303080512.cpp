...
void Library::loadData() {
    ...
    while (getline(file, line)) {
        // Validate if the file is empty. If the line is empty, continue loading data.
        if (line.empty() == true) continue;
        else if (line.empty() == false) {
            cout << "File is not empty. File contains: " << line << endl;
        ...
        // Parse data from temporarily stored data in data members from the book class
        getline(ss, genreStr, ',');
        getline(ss, title, ',');
        getline(ss, author, ',');
        getline(ss, typeStr, ',');
